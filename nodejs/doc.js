...
/*
Important: connection.write() will only accept raw APDU payloads and a DPT. This practically
means that for reading and writing to anything other than a binary switch (eg. for dimmer controls)
you'll need to declare one or more datapoints. Declare datapoints based on their DPT

Datapoints correlate an endpoint (identifed by a group address such as '1/2/3') with a DPT (DataPoint Type), 
so that serialization of values to and from KNX works correctly (eg. temperatures as 16bit floats), and values
are being translated to Javascript objects and back.
...
/*
Datapoints need to be bound to a connection. This can be done either at their creation, 
or using their bind() call. Its important to highlight that before you start defining datapoints
(and devices as we'll see later), your code needs to ensure that the connection has been established, 
usually by declaring them in the 'connected' handler:
...
The third (optional) parameter bitlength is necessary for datapoint types where the bitlength does not equal the buffers bytelength * 8. 
This is the case for dpt 1 (bitlength 1), 2 (bitlength 2) and 3 (bitlength 4). For other dpts the paramter can be omitted.
